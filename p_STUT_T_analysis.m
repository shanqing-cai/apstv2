...
for i1 = 1 : numel(perts)
    ...
    for i2 = 1 : numel(groups)
        ...
        errorbar(1 : nInts, dat_mean, dat_sem, ...
                 lineTypes.(grp), 'Color', colors.(grp), 'LineWidth', lineWidths.(grp));
    ...
    if i1 == 1
        ...
        text(xs(1) + (lgdX + 0.60 * lgdW) * range(xs), ys(1) + (lgdY + 0.725 * lgdH) * range(ys), ...
             'PFS', 'FontSize', bpFontSize * 0.75, 'FontAngle', 'italic', 'Color', colors.PFS);
        text(xs(1) + (lgdX + 0.825 * lgdW) * range(xs), ys(1) + (lgdY + 0.725 * lgdH) * range(ys), ...
...
%              lineTypes.PFS, 'LineWidth', lineWidths.PFS, 'Color', colors.PFS);
%         plot(xs(1) + (lgdX + [0.85, 0.95] * lgdW) * range(xs), ...
%              repmat(ys(1) + (lgdY + 0.55 * lgdH) * range(ys), 1, 2), ...
%              lineTypes.PWS, 'LineWidth', lineWidths.PWS, 'Color', colors.PWS);
%         plot(xs(1) + (lgdX + 0.75 * lgdW) * range(xs), ys(1) + (lgdY + 0.55 * lgdH) * range(ys), 'o', ...
%              'MarkerEdgeColor', colors.PFS, 'MarkerFaceColor', [1, 1, 1]);
         
        plot(xs(1) + (lgdX + [0.60, 0.75] * lgdW) * range(xs), ...
             repmat(ys(1) + (lgdY + 0.55 * lgdH) * range(ys), 1, 2), ...
             lineTypes.PFS, 'LineWidth', lineWidths.PFS, 'Color', colors.PFS);
        plot(xs(1) + (lgdX + [0.825, 0.975] * lgdW) * range(xs), ...
             repmat(ys(1) + (lgdY + 0.55 * lgdH) * range(ys), 1, 2), ...
             lineTypes.PWS, 'LineWidth', lineWidths.PWS, 'Color', colors.PWS);
        plot(xs(1) + (lgdX + 0.675 * lgdW) * range(xs), ys(1) + (lgdY + 0.55 * lgdH) * range(ys), 'o', ...
             'MarkerEdgeColor', colors.PFS, 'MarkerFaceColor', [1, 1, 1]);
        plot(xs(1) + (lgdX + 0.900 * lgdW) * range(xs), ys(1) + (lgdY + 0.55 * lgdH) * range(ys), 'o', ...
        ...
        plot(xs(1) + (lgdX + [0.825, 0.975] * lgdW) * range(xs), ...
             repmat(ys(1) + (lgdY + 0.40 * lgdH) * range(ys), 1, 2), ...
             lineTypes.PWS, 'LineWidth', lineWidths.PWS, 'Color', colors.PWS);
        plot(xs(1) + (lgdX + 0.675 * lgdW) * range(xs), ys(1) + (lgdY + 0.40 * lgdH) * range(ys), 'o', ...
             'MarkerEdgeColor', colors.PFS, 'MarkerFaceColor', colors.PFS);
        plot(xs(1) + (lgdX + 0.900 * lgdW) * range(xs), ys(1) + (lgdY + 0.40 * lgdH) * range(ys), 'o', ...
